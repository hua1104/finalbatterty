...
def camera_thread():
    ...
    # 2. 物理级曝光锁定
    # ==========================================
    print(">> 正在向底层驱动注入黄金曝光参数: 300...")
    os.system("v4l2-ctl -d /dev/video0 -c exposure_auto=1")
    os.system("v4l2-ctl -d /dev/video0 -c exposure_absolute=450")
    
    time.sleep(1) 
    print(">> 摄像头初始化完毕，进入主循环！")
...
def worker_thread():
    ...
    if HARDWARE_AVAILABLE:
        ...
        class Mock: 
            ...
            def eject(self, angle=90): print(f"[模拟] 拨片执行 {angle} 度分拣"); time.sleep(0.5)
    ...
    while not sys_state["exit"]:
        ...
        if detected and not last_detect:
            if (curr_time - last_trigger_time) > COOLDOWN_SECONDS:
                ...
                with lock:
                    ...
                    sys_state["log"] = f"[{time.strftime('%H:%M:%S')}] 定点拍照，AI分析中..."
                ...
                if detector:
                    ...
                    res_str = "Wrinkle" if is_bad else "Normal"
                    conf = 0.95
                ...
                # 使用 .lower() 增加容错性，适配英文返回
                res_lower = res_str.lower()
                if "wrinkle" in res_lower or "褶皱" in res_lower:
                    is_bad = True
                    detail = "褶皱"
                    target_angle = 40  # 褶皱拨动 40°
                elif "scratch" in res_lower or "划痕" in res_lower:
                    is_bad = True
                    detail = "划痕"
                    target_angle = 75  # 划痕拨动 75°
                elif "normal" in res_lower or "合格" in res_lower:
                    is_bad = False
                    detail = "正常"
                else:
                    is_bad = is_bad # 保持检测器的 is_bad 结果
                    detail = "其他" if is_bad else "正常"
                    target_angle = 90 if is_bad else 0

                # 更新 UI 结果
                path = save_snapshot(ann_img)
                res_rgb = cv2.cvtColor(ann_img, cv2.COLOR_BGR2RGB)
                curr_t_str = time.strftime("%H:%M:%S")

                with lock:
                    sys_state["result_frame"] = res_rgb 
                    sys_state["res"] = f"{detail} ({res_str})"
                    sys_state["conf"] = conf
                    sys_state["log"] = f"[{curr_t_str}] 判定: {detail}，需拨动 {target_angle}°"
                    
                    if is_bad: 
                        sys_state["bad"] += 1
                        sys_state["defects"][detail] = sys_state["defects"].get(detail, 0) + 1
                    else: 
                        sys_state["good"] += 1
                    
                    sys_state["new_records"].append({
                        "time": curr_t_str, "result": detail, 
                        "detail": res_str, "conf": f"{int(conf*100)}%"
                    })
        ...
        for i in range(len(active_items)-1, -1, -1):
            ...
            if item['is_bad'] and item['steps'] >= SORT_STEPS:
                push_angle = item.get('angle', 90)
                push_type = item.get('type', '次品')

                with lock: 
                    sys_state["log"] = f"[{time.strftime('%H:%M:%S')}] {push_type}到位，执行剔除"
                
                # ====== 终极调试雷达 ======
                print(f"\n>>>>>>>> 准备触发物理剔除！当前硬件状态: HARDWARE_AVAILABLE = {HARDWARE_AVAILABLE} <<<<<<<<")
                
                if HARDWARE_AVAILABLE: 
                    try:
                        print(f"尝试执行高级分拣 (角度: {push_angle})...")
                        pusher.eject(angle=push_angle)
                        print("✅ 高级分拣动作完成！")
                    except TypeError:
                        try:
                            print("⚠️ 当前驱动不支持角度参数，尝试降级执行基础 eject()...")
                            pusher.eject()
                            print("✅ 基础分拣动作完成！")
                        except Exception as e:
                            print(f"❌ 致命错误：基础分拣失败，驱动底层报错 -> {e}")
                    except Exception as e:
                        print(f"❌ 致命错误：高级分拣失败，驱动底层报错 -> {e}")
                else: 
                    print(f"⚠️ [模拟模式警告] 没有检测到真实硬件！假设舵机已经转了 {push_angle} 度。")
                    time.sleep(0.5)
...
class VirtualKeyboard(tk.Toplevel):
    def __init__(self, target_entry, root):
        ...
        bar = tk.Frame(self, bg=THEME["bg_dark"], height=35)
        ...
        tk.Button(bar, text="⬇ 收起", bg=THEME["ec_red"], fg="white", font=("微软雅黑", 10, "bold"), relief="flat", command=self.destroy).pack(side="right", padx=5)

        self.key_area = tk.Frame(self, bg=THEME["bg_panel"])
        self.key_area.pack(fill="both", expand=True, padx=5, pady=2)
        
        func_f = tk.Frame(self, bg=THEME["bg_panel"])
        func_f.pack(fill="x", padx=5, pady=5)
        
        self.btn_shift = tk.Button(func_f, text="大小写", width=8, height=1, bg=THEME["border"], fg="white", font=("微软雅黑", 10), relief="flat", command=self.toggle_shift)
        self.btn_shift.pack(side="left", padx=2)
        
        tk.Button(func_f, text="—— 空 格 ——", width=20, height=1, bg=THEME["bg_dark"], fg="white", relief="flat", command=lambda: self.add_char(" ")).pack(side="left", padx=5, expand=True, fill="x")
        
        tk.Button(func_f, text="← 退格", width=8, height=1, bg=THEME["ec_yellow"], fg="black", font=("微软雅黑", 10, "bold"), relief="flat", command=self.backspace).pack(side="left", padx=2)
        
        tk.Button(func_f, text="确定", width=8, height=1, bg=THEME["ec_green"], fg="black", font=("微软雅黑", 10, "bold"), relief="flat", command=self.destroy).pack(side="right", padx=2)

        self.render_keys()
        self.lift(); self.focus_force()

    def create_tab_btn(self, parent, text, mode):
        return tk.Button(parent, text=text, font=("微软雅黑", 10, "bold"), bg=THEME["bg_panel"], fg=THEME["text_dim"], relief="flat", padx=10, command=lambda: self.switch_mode(mode))
    ...
    def render_keys(self):
        ...
        for row in layout:
            ...
            for char in row:
                f = ("微软雅黑", 12, "bold") if char in "0123456789" else ("微软雅黑", 12)
...
class ParticleSystem:
    def __init__(self, canvas, width, height, count=30): # 减少粒子数降低小屏负担
...
class App:
    ...
    def setup_styles(self):
        ...
        # 缩小树的行高和字体
        style.configure("Treeview", background=THEME["bg_panel"], foreground=THEME["text_main"], fieldbackground=THEME["bg_panel"], borderwidth=0, rowheight=35, font=("微软雅黑", 10))
        style.configure("Treeview.Heading", background=THEME["bg_dark"], foreground=THEME["ec_cyan"], font=("微软雅黑", 11, "bold"), relief="flat")
    ...
    def build_login_ui(self):
        ...
        # 字体全部调小，适配小屏
        self.lbl_login_time = tk.Label(self.root, text="--:--:--", font=("Arial", 24, "bold"), bg=THEME["bg_dark"], fg=THEME["text_dim"])
        self.lbl_login_time.place(relx=0.5, rely=0.15, anchor="center")
        self.lbl_login_date = tk.Label(self.root, text="----", font=("微软雅黑", 14), bg=THEME["bg_dark"], fg=THEME["text_dim"])
        self.lbl_login_date.place(relx=0.5, rely=0.22, anchor="center")

        center = tk.Frame(self.root, bg=THEME["bg_dark"])
        center.place(relx=0.5, rely=0.40, anchor="center") # 稍微往下移一点
        
        tk.Label(center, text="基于深度学习的软包锂电池缺陷检测与分类系统", font=("微软雅黑", 24, "bold"), bg=THEME["bg_dark"], fg="white").pack(pady=5)
        tk.Label(center, text="智能制造系统终端", font=("微软雅黑", 12, "bold"), bg=THEME["bg_dark"], fg=THEME["ec_blue"]).pack(pady=(0, 20))
        
        card = tk.Frame(center, bg=THEME["bg_panel"], padx=30, pady=25, highlightbackground=THEME["ec_blue"], highlightthickness=1)
        card.pack()
        
        tk.Label(card, text="操作人员", font=("微软雅黑", 10), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(anchor="w")
        row1 = tk.Frame(card, bg=THEME["bg_panel"]); row1.pack(fill="x", pady=(5, 10))
        self.en_user = tk.Entry(row1, font=("微软雅黑", 14), bg=THEME["bg_dark"], fg="white", relief="flat", insertbackground="white", width=18)
        self.en_user.pack(side="left", ipady=6); self.en_user.insert(0, "admin")
        self.en_user.bind("<Button-1>", lambda e: self.open_kb(e, self.en_user))
        tk.Button(row1, text="⌨", bg=THEME["border"], fg="white", width=4, font=("Arial", 12), command=lambda: self.open_kb(None, self.en_user)).pack(side="right", fill="y")
        
        tk.Label(card, text="身份验证码", font=("微软雅黑", 10), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(anchor="w")
        row2 = tk.Frame(card, bg=THEME["bg_panel"]); row2.pack(fill="x", pady=(5, 20))
        self.en_pwd = tk.Entry(row2, show="●", font=("微软雅黑", 14), bg=THEME["bg_dark"], fg="white", relief="flat", insertbackground="white", width=18)
        self.en_pwd.pack(side="left", ipady=6); self.en_pwd.insert(0, "123")
        self.en_pwd.bind("<Button-1>", lambda e: self.open_kb(e, self.en_pwd))
        tk.Button(row2, text="⌨", bg=THEME["border"], fg="white", width=4, font=("Arial", 12), command=lambda: self.open_kb(None, self.en_pwd)).pack(side="right", fill="y")
        
        self.btn_login = tk.Button(card, text="进入系统", bg=THEME["ec_blue"], fg="white", font=("微软雅黑", 14, "bold"), relief="flat", command=self.do_login)
    ...
    def build_dashboard(self):
        self.clear_window()
        self.sidebar = tk.Frame(self.root, bg=THEME["bg_panel"], width=200) # 缩小侧边栏
        self.sidebar.pack(side="left", fill="y"); self.sidebar.pack_propagate(False)
        
        tk.Label(self.sidebar, text="❖ 智能中控", font=("微软雅黑", 18, "bold"), bg=THEME["bg_panel"], fg=THEME["ec_blue"]).pack(pady=(30, 30))
        self.btns = {}
        ...
        bot.pack(side="bottom", fill="x", pady=20, padx=15)
        tk.Label(bot, text=f"操作员: {sys_state['user']}", font=("微软雅黑", 12, "bold"), bg=THEME["bg_panel"], fg="white").pack(anchor="w", pady=(0, 15))
        tk.Button(bot, text="退出系统", bg=THEME["ec_red"], fg="white", relief="flat", font=("微软雅黑", 11), command=self.root.destroy).pack(fill="x", ipady=8)

        main = tk.Frame(self.root, bg=THEME["bg_dark"])
        main.pack(side="right", fill="both", expand=True)
        
        header = tk.Frame(main, bg=THEME["bg_dark"], height=60) # 缩小头部高度
        header.pack(side="top", fill="x", padx=20, pady=10); header.pack_propagate(False)
        self.lbl_title = tk.Label(header, text="双流实时监控", font=("微软雅黑", 20, "bold"), bg=THEME["bg_dark"], fg="white")
        self.lbl_title.pack(side="left")
        
        stat_f = tk.Frame(header, bg=THEME["bg_dark"]); stat_f.pack(side="right")
        self.lbl_time = tk.Label(stat_f, text="00:00:00", font=("Arial", 20, "bold"), bg=THEME["bg_dark"], fg=THEME["ec_blue"])
        self.lbl_time.pack(side="right", padx=(20, 0))

        self.container = tk.Frame(main, bg=THEME["bg_dark"])
        self.container.pack(side="bottom", fill="both", expand=True, padx=20, pady=(0, 20))
    ...
    def create_nav_btn(self, parent, text, tag):
        b = tk.Button(parent, text=text, font=("微软雅黑", 12), bg=THEME["bg_panel"], fg=THEME["text_dim"], 
                      relief="flat", anchor="w", padx=30, command=lambda: self.show_page(f"{tag}Page"))
        b.pack(fill="x", ipady=12, pady=4)
    ...
    def show_page(self, name):
        ...
        for n, b in self.btns.items():
            if n == name: b.config(bg=THEME["bg_dark"], fg=THEME["ec_blue"], font=("微软雅黑", 12, "bold"))
            else: b.config(bg=THEME["bg_panel"], fg=THEME["text_dim"], font=("微软雅黑", 12, "normal"))
...
class MonitorPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(f1, text="■ 实时监控画面", bg="black", fg="white", font=("微软雅黑", 10)).place(x=10, y=10)
        self.lbl_live = tk.Label(f1, bg="black", text="无视频信号", fg=THEME["text_dim"], font=("微软雅黑", 12))
        ...
        tk.Label(f2, text="❖ 智能缺陷锁定", bg="#1a1a1a", fg=THEME["ec_cyan"], font=("微软雅黑", 10, "bold")).place(x=10, y=10)
        self.lbl_res_img = tk.Label(f2, bg="#1a1a1a", text="等待检测...", fg=THEME["text_dim"], font=("微软雅黑", 12))
        ...
        tk.Label(c1, text="AI 智能判定", fg=THEME["text_dim"], bg=THEME["bg_panel"], font=("微软雅黑", 10)).pack(pady=(15,5))
        self.lbl_text_res = tk.Label(c1, text="系统待命", font=("黑体", 28, "bold"), fg="white", bg=THEME["bg_panel"])
        self.lbl_text_res.pack()
        self.bar = ttk.Progressbar(c1, style="Horizontal.TProgressbar", length=140, mode="determinate")
        ...
        tk.Label(c2, text="系统运行日志", fg=THEME["text_dim"], bg=THEME["bg_panel"], font=("微软雅黑", 10)).pack(anchor="w", padx=10, pady=5)
        self.log_box = st.ScrolledText(c2, bg="#0f172a", fg=THEME["ec_green"], font=("Consolas", 10), relief="flat", state="disabled")
        self.log_box.pack(fill="both", expand=True, padx=10, pady=(0,10))
        
        c3 = tk.Frame(bot_frame, bg=THEME["bg_dark"])
        c3.place(relx=0.79, rely=0, relwidth=0.21, relheight=1)
        self.btn_run = tk.Button(c3, text="启动", bg="#2F5D34", fg="white", font=("微软雅黑", 18, "bold"), relief="flat", command=self.toggle)
...
class DataPage(tk.Frame):
    ...
    def make_stat(self, parent, title, val, col):
        ...
        tk.Label(f, text=title, font=("微软雅黑", 11), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(pady=(15, 5))
        l = tk.Label(f, text=val, font=("Arial", 28, "bold"), bg=THEME["bg_panel"], fg="white")
        l.pack(pady=5); return l

    def make_chart_card(self, parent, title, col):
        f = tk.Frame(parent, bg=THEME["bg_panel"], highlightbackground=THEME["border"], highlightthickness=1)
        f.place(relx=0.51*col, rely=0, relwidth=0.49, relheight=1)
        tk.Label(f, text=title, font=("微软雅黑", 12, "bold"), bg=THEME["bg_panel"], fg=THEME["text_main"]).pack(pady=10)
    ...
    def update_data(self):
        ...
        if w>10:
            ...
            self.c1.create_oval(x-r, y-r, x+r, y+r, outline=THEME["grid_line"], width=15)
            if rate>0: self.c1.create_arc(x-r, y-r, x+r, y+r, start=90, extent=-360*rate, style="arc", outline=THEME["ec_green"], width=15)
            self.c1.create_text(x,y-10, text=f"{int(rate*100)}%", fill="white", font=("Arial", 24, "bold"))
            self.c1.create_text(x,y+25, text="综合良率", fill=THEME["text_dim"], font=("微软雅黑", 10))
            self.c1.create_rectangle(x-30, h-30, x-20, h-20, fill=THEME["ec_green"], outline="")
            self.c1.create_text(x-15, h-25, text="合格", anchor="w", fill=THEME["text_dim"], font=("微软雅黑", 9))
        ...
        if w>10:
            ...
            for i, (k,v) in enumerate(items):
                ...
                self.c2.create_text(cx, base+15, text=k, fill=THEME["text_dim"], font=("微软雅黑", 10))
                self.c2.create_text(cx, base-ch-10, text=str(v), fill="white", font=("Arial", 10))
...
class StatusPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i, (title, key) in enumerate([("云端连接", "cloud"), ("红外传感器", "sensor"), ("电机驱动", "motor"), ("摄像头信号", "camera")]):
            ...
            tk.Label(f, text=title, font=("微软雅黑", 12), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(pady=8)
            lbl = tk.Label(f, text="--", font=("Arial", 18, "bold"), bg=THEME["bg_panel"], fg="white")
            lbl.pack()
        ...
        tk.Label(bot, text="系统核心负载 (CPU Load)", font=("微软雅黑", 12), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(anchor="w", padx=15, pady=10)
        self.cpu_bar = ttk.Progressbar(bot, style="Horizontal.TProgressbar", length=600, mode="determinate")
        self.cpu_bar.pack(pady=10)
        self.lbl_disk = tk.Label(bot, text="剩余存储: --", font=("微软雅黑", 10), bg=THEME["bg_panel"], fg="white")
        self.lbl_disk.pack()
...
class HistoryPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(container, text="最近 50 条追溯记录", font=("微软雅黑", 14, "bold"), bg=THEME["bg_panel"], fg="white").pack(anchor="w", padx=20, pady=15)
        
        cols = ("time", "res", "detail", "conf")
        self.tree = ttk.Treeview(container, columns=cols, show="headings")
        self.tree.heading("time", text="检测时间"); self.tree.column("time", width=180, anchor="center")
        self.tree.heading("res", text="AI 判定"); self.tree.column("res", width=100, anchor="center")
        self.tree.heading("detail", text="缺陷类型"); self.tree.column("detail", width=130, anchor="center")
        ...
        scrolly.pack(side="right", fill="y", pady=(0, 20), padx=(0, 20))
