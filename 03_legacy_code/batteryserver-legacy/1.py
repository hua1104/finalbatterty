...
def camera_thread():
    ...
    os.system("v4l2-ctl -d /dev/video0 -c exposure_absolute=330")
    time.sleep(1) 
    ...
    while not sys_state["exit"]:
        ...
        if detected and not last_detect:
            if (curr_time - last_trigger_time) > COOLDOWN_SECONDS:
                ...
                if "wrinkle" in res_lower or "褶皱" in res_lower:
                    is_bad = True; detail = "褶皱"; target_angle = 40 
                elif "scratch" in res_lower or "划痕" in res_lower:
                    is_bad = True; detail = "划痕"; target_angle = 75 
                elif "normal" in res_lower or "合格" in res_lower:
                    is_bad = False; detail = "正常"
                else:
                    is_bad = is_bad; detail = "其他" if is_bad else "正常"
                ...
                active_items.append({
                    'is_bad': is_bad, 'steps': 0, 'angle': target_angle, 'type': detail
                })
        ...
        for i in range(len(active_items)-1, -1, -1):
            ...
            if item['is_bad'] and item['steps'] >= SORT_STEPS:
                ...
                with lock: sys_state["log"] = f"[{time.strftime('%H:%M:%S')}] {push_type}到位，执行 {push_angle}° 剔除"
                if HARDWARE_AVAILABLE: pusher.eject(angle=push_angle)
                else: time.sleep(0.5)
                active_items.pop(i)
...
class VirtualKeyboard(tk.Toplevel):
    def __init__(self, target_entry, root):
        ...
        self.layouts = {
            "en_lower": [['1','2','3','4','5','6','7','8','9','0'],['q','w','e','r','t','y','u','i','o','p'],['a','s','d','f','g','h','j','k','l'],['z','x','c','v','b','n','m'],[',','.','@','/','-','_','+','=']],
            "en_upper": [['1','2','3','4','5','6','7','8','9','0'],['Q','W','E','R','T','Y','U','I','O','P'],['A','S','D','F','G','H','J','K','L'],['Z','X','C','V','B','N','M'],['!','?','#','$','%','&','*','(',')']],
        ...
        self.render_keys(); self.lift(); self.focus_force()
    ...
    def render_keys(self):
        ...
        for row in layout:
            ...
            for char in row:
                ...
                btn = tk.Button(row_f, text=char, font=f, bg=THEME["bg_dark"], fg="white", relief="flat", activebackground=THEME["ec_blue"], activeforeground="white", command=lambda c=char: self.add_char(c))
...
class ParticleSystem:
    def __init__(self, canvas, width, height, count=50):
        ...
        for _ in range(count):
            self.particles.append({"x": random.randint(0, width), "y": random.randint(0, height), "vx": random.uniform(-0.8, 0.8), "vy": random.uniform(-0.8, 0.8), "r": random.randint(2, 4)})
    def animate(self):
        try:
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Intelligent Sorter V26")
    ...
    def build_login_ui(self):
        ...
        tk.Label(center, text="锂电池外观缺陷检测与智能分拣系统", font=("微软雅黑", 42, "bold"), bg=THEME["bg_dark"], fg=THEME["text_main"]).pack(pady=10)
        ...
        self.en_user = tk.Entry(row1, font=("微软雅黑", 18), bg=THEME["bg_dark"], fg=THEME["text_main"], relief="flat", insertbackground=THEME["text_main"], width=18)
        ...
        tk.Button(row1, text="⌨", bg=THEME["border"], fg=THEME["text_main"], width=4, font=("Arial", 14), command=lambda: self.open_kb(None, self.en_user)).pack(side="right", fill="y")
        ...
        self.en_pwd = tk.Entry(row2, show="●", font=("微软雅黑", 18), bg=THEME["bg_dark"], fg=THEME["text_main"], relief="flat", insertbackground=THEME["text_main"], width=18)
        self.en_pwd.pack(side="left", ipady=8); self.en_pwd.insert(0, "123")
        self.en_pwd.bind("<Button-1>", lambda e: self.open_kb(e, self.en_pwd))
        tk.Button(row2, text="⌨", bg=THEME["border"], fg=THEME["text_main"], width=4, font=("Arial", 14), command=lambda: self.open_kb(None, self.en_pwd)).pack(side="right", fill="y")
        self.btn_login = tk.Button(card, text="进入系统", bg=THEME["ec_blue"], fg="white", font=("微软雅黑", 18, "bold"), relief="flat", command=self.do_login)
        self.btn_login.pack(fill="x", ipady=12); self.update_clock(True)

    def animate_bg(self):
        if hasattr(self, 'particles'): self.particles.animate(); self.root.after(50, self.animate_bg)
    ...
    def do_login(self):
        ...
        if verify_login_on_server(self.en_user.get(), self.en_pwd.get()):
            sys_state["user"] = self.en_user.get(); self.build_dashboard()
            threading.Thread(target=worker_thread, daemon=True).start()
            threading.Thread(target=camera_thread, daemon=True).start()
        else: messagebox.showerror("拒绝访问", "验证失败：账号或密码错误")

    def toggle_theme(self):
        global THEME
        THEME = LIGHT_THEME if THEME["name"] == "dark" else DARK_THEME
        self.root.configure(bg=THEME["bg_dark"])
        self.setup_styles()
        self.refresh_ui(self.root)
        # 强制 Canvas 重绘
        for page in self.pages.values(): 
            if hasattr(page, 'update_data'): page.update_data()

    def refresh_ui(self, container):
        for widget in container.winfo_children():
            try:
                w_class = widget.winfo_class()
                if w_class == 'Frame': widget.configure(bg=THEME["bg_panel"] if widget == self.sidebar else THEME["bg_dark"])
                elif w_class == 'Label':
                    # 排除特定颜色的状态标签
                    if widget.cget("foreground") not in [DARK_THEME["ec_green"], LIGHT_THEME["ec_green"], DARK_THEME["ec_red"], LIGHT_THEME["ec_red"]]:
                        widget.configure(bg=widget.master.cget("bg"), fg=THEME["text_main"])
                    else:
                        widget.configure(bg=widget.master.cget("bg"))
                elif w_class == 'Button':
                    # 排除功能性按钮
                    if "Page" in str(widget): continue 
                    if widget.cget("text") not in ["启动", "停止", "退出系统"]:
                        widget.configure(bg=THEME["bg_panel"], fg=THEME["text_main"])
                elif w_class == 'Text' or w_class == 'ScrolledText':
                    widget.configure(bg="#0f172a" if THEME["name"] == "dark" else "#f8fafc", fg=THEME["ec_green"])
            except: pass
            if widget.winfo_children(): self.refresh_ui(widget)
    ...
    def build_dashboard(self):
        ...
        tk.Label(bot, text=f"操作员: {sys_state['user']}", font=("微软雅黑", 14, "bold"), bg=THEME["bg_panel"], fg=THEME["text_main"]).pack(anchor="w", pady=(0, 15))
        
        # 主题切换按钮
        tk.Button(bot, text="🌓 切换主题模式", bg=THEME["border"], fg=THEME["text_main"], relief="flat", font=("微软雅黑", 12), command=self.toggle_theme).pack(fill="x", ipady=8, pady=(0, 10))
        ...
        self.lbl_title = tk.Label(header, text="双流实时监控", font=("微软雅黑", 26, "bold"), bg=THEME["bg_dark"], fg=THEME["text_main"])
        ...
        for P in (MonitorPage, DataPage, HistoryPage, StatusPage):
            f = P(parent=self.container, controller=self); self.pages[P.__name__] = f; f.grid(row=0, column=0, sticky="nsew")
        self.show_page("MonitorPage"); self.update_loop()

    def create_nav_btn(self, parent, text, tag):
        b = tk.Button(parent, text=text, font=("微软雅黑", 15), bg=THEME["bg_panel"], fg=THEME["text_dim"], relief="flat", anchor="w", padx=50, command=lambda: self.show_page(f"{tag}Page"))
        b.pack(fill="x", ipady=18, pady=4); self.btns[f"{tag}Page"] = b
    ...
    def open_kb(self, event, widget):
        ...
        self.kb_win = VirtualKeyboard(widget, self.root); return "break"
...
class MonitorPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.lbl_text_res = tk.Label(c1, text="系统待命", font=("黑体", 42, "bold"), fg=THEME["text_main"], bg=THEME["bg_panel"])
    ...
    def update_data(self):
        if sys_state["live_frame"] is not None:
            try:
                ...
                self.tk_live = ImageTk.PhotoImage(img); self.lbl_live.config(image=self.tk_live, text="")
            except: pass
        if sys_state["result_frame"] is not None:
            try:
                img = Image.fromarray(sys_state["result_frame"]).resize((550, 310))
                self.tk_res = ImageTk.PhotoImage(img); self.lbl_res_img.config(image=self.tk_res, text="")
            except: pass
        res = sys_state["res"]
        color = THEME["ec_green"] if "正常" in res or "合格" in res else (THEME["ec_red"] if any(x in res for x in ["褶皱", "划痕", "其他"]) else THEME["text_main"])
        self.lbl_text_res.config(text=res, fg=color)
...
class DataPage(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent, bg=THEME["bg_dark"])
        stat_row = tk.Frame(self, bg=THEME["bg_dark"]); stat_row.pack(fill="x", pady=(0, 20))
        ...
        chart_row = tk.Frame(self, bg=THEME["bg_dark"]); chart_row.pack(fill="both", expand=True)
        self.c1 = self.make_chart_card(chart_row, "良品率分析图", 0); self.c2 = self.make_chart_card(chart_row, "缺陷分布直方图", 1)

    def make_stat(self, parent, title, val, col):
        f = tk.Frame(parent, bg=THEME["bg_panel"], highlightbackground=THEME["border"], highlightthickness=1); f.place(relx=col*0.34, rely=0, relwidth=0.32, relheight=1)
        tk.Label(f, text=title, font=("微软雅黑", 13), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(pady=(20, 5))
        l = tk.Label(f, text=val, font=("Arial", 40, "bold"), bg=THEME["bg_panel"], fg=THEME["text_main"]); l.pack(pady=5); return l

    def make_chart_card(self, parent, title, col):
        f = tk.Frame(parent, bg=THEME["bg_panel"], highlightbackground=THEME["border"], highlightthickness=1); f.place(relx=0.51*col, rely=0, relwidth=0.49, relheight=1)
        tk.Label(f, text=title, font=("微软雅黑", 15, "bold"), bg=THEME["bg_panel"], fg=THEME["text_main"]).pack(pady=15)
        c = tk.Canvas(f, bg=THEME["bg_panel"], highlightthickness=0); c.pack(fill="both", expand=True); return c
    ...
    def update_data(self):
        ...
        self.c1.delete("all"); self.c1.config(bg=THEME["bg_panel"])
        ...
        if w>10:
            ...
            self.c1.create_text(x,y-10, text=f"{int(rate*100)}%", fill=THEME["text_main"], font=("Arial", 36, "bold"))
            self.c1.create_text(x,y+30, text="综合良率", fill=THEME["text_dim"], font=("微软雅黑", 12))

        self.c2.delete("all"); self.c2.config(bg=THEME["bg_panel"])
        w, h = self.c2.winfo_width(), self.c2.winfo_height()
        if w>10:
            d = sys_state["defects"]; items = list(d.items()); m = max(d.values()) if any(d.values()) else 1
            bw = w/(len(items)*3); sp = w/len(items); colors = [THEME["ec_blue"], THEME["ec_yellow"], THEME["ec_purple"]]
            for i in range(5):
                line_y = (h-60) * (i/4) + 20
            ...
            for i, (k,v) in enumerate(items):
                ...
                self.c2.create_text(cx, base-ch-15, text=str(v), fill=THEME["text_main"], font=("Arial", 12))
...
class StatusPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i, (title, key) in enumerate([("云端连接", "cloud"), ("红外传感器", "sensor"), ("电机驱动", "motor"), ("摄像头信号", "camera")]):
            row, col = divmod(i, 2)
            f = tk.Frame(self, bg=THEME["bg_panel"], highlightbackground=THEME["border"], highlightthickness=1); f.place(relx=col*0.5+0.02, rely=row*0.3+0.05, relwidth=0.46, relheight=0.25)
            tk.Label(f, text=title, font=("微软雅黑", 14), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(pady=10)
            lbl = tk.Label(f, text="--", font=("Arial", 24, "bold"), bg=THEME["bg_panel"], fg=THEME["text_main"]); lbl.pack(); setattr(self, f"lbl_{key}", lbl)
        bot = tk.Frame(self, bg=THEME["bg_panel"], highlightbackground=THEME["border"], highlightthickness=1); bot.place(relx=0.02, rely=0.6, relwidth=0.96, relheight=0.35)
        tk.Label(bot, text="系统核心负载 (CPU Load)", font=("微软雅黑", 14), bg=THEME["bg_panel"], fg=THEME["text_dim"]).pack(anchor="w", padx=20, pady=15)
        self.cpu_bar = ttk.Progressbar(bot, style="Horizontal.TProgressbar", length=800, mode="determinate"); self.cpu_bar.pack(pady=20)
        self.lbl_disk = tk.Label(bot, text="剩余存储: --", font=("微软雅黑", 12), bg=THEME["bg_panel"], fg=THEME["text_main"]); self.lbl_disk.pack()
    ...
    def update_data(self):
        ...
        if not HARDWARE_AVAILABLE: self.lbl_motor.config(text="模拟运行" if h["motor"] else "模拟待机", fg=THEME["ec_purple"] if h["motor"] else THEME["text_dim"])
        else: self.lbl_motor.config(text="运转中" if h["motor"] else "待机停转", fg=THEME["ec_green"] if h["motor"] else THEME["ec_yellow"])
        self.lbl_camera.config(text="正常" if h["camera"] else "丢失", fg=THEME["ec_green"] if h["camera"] else THEME["ec_red"])
        self.cpu_bar["value"] = h["cpu_load"]; self.lbl_disk.config(text=f"剩余存储: {h['disk_free']}")

class HistoryPage(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent, bg=THEME["bg_dark"])
        container = tk.Frame(self, bg=THEME["bg_panel"], highlightbackground=THEME["border"], highlightthickness=1); container.pack(fill="both", expand=True)
        tk.Label(container, text="最近 50 条追溯记录", font=("微软雅黑", 15, "bold"), bg=THEME["bg_panel"], fg=THEME["text_main"]).pack(anchor="w", padx=25, pady=25)
        cols = ("time", "res", "detail", "conf")
        self.tree = ttk.Treeview(container, columns=cols, show="headings")
        for c, t in zip(cols, ["检测时间", "AI 判定", "缺陷类型", "置信度"]): self.tree.heading(c, text=t); self.tree.column(c, width=150, anchor="center")
        self.tree.pack(side="left", fill="both", expand=True, padx=(25, 0), pady=(0, 25))
        scrolly = ttk.Scrollbar(container, orient="vertical", command=self.tree.yview); scrolly.pack(side="right", fill="y", pady=(0, 25), padx=(0, 25)); self.tree.configure(yscroll=scrolly.set)
        self.tree.tag_configure("ok", foreground=THEME["ec_green"]); self.tree.tag_configure("ng", foreground=THEME["ec_red"])

    def update_data(self):
        if sys_state["new_records"]:
            for r in sys_state["new_records"]:
                tag = "ok" if "正常" in r["result"] else "ng"
